...
//            mMuxer.writeSampleData(mVideoTrackIndex, encodedData, mBufferInfo);//写入
    ...
    public void showForegroundNotification() {
        NotificationCompat.Action action = new NotificationCompat.Action(
                R.drawable.ic_close_pushing_screen,
                "关闭",
                PendingIntent.getBroadcast(
                        getApplicationContext(),
                        10000,
                        new Intent(ACTION_CLOSE_PUSHING_SCREEN),
                        FLAG_CANCEL_CURRENT
                )
        );
        Notification notification = new NotificationCompat.Builder(PushScreenService.this,
                "com.hevin.pushscreen.NOTIFICATION_CHANNEL_1")
                .setContentTitle(getString(R.string.screen_pushing))
                .setSmallIcon(R.drawable.ic_pusher_screen_pushing)
                .addAction(action)
                .build();
